...
def classify(logs):
    ...
    for source, log_msg in logs:
        labels.append((source, classify_log(source, log_msg)))  # Pass both source and log_msg
    return labels 


def classify_log(source, log_message):
    if source == "LegacyCRM":
        label = classify_with_llm(log_message)  # Call classify_with_llm with log_message
    else:
        label = classify_with_regex(log_message)
        if label is None:
            label = classify_with_bert(log_message)  # Call classify_with_bert with log_message
    return label
